...
def reactorModel(fuelCompo, FPCompo, t_final, n_th_init, n_fa_init, mTot):
    ...
    N_U235 = mTot * frac_U235 / mM.molarMass('U235') * AVOGADRO
    N_U238 = mTot * frac_U238 / mM.molarMass('U238') * AVOGADRO
    N_Pu239 = mTot * frac_Pu239 / mM.molarMass('Pu239') * AVOGADRO
    N_Th232 = mTot * frac_Th232 / mM.molarMass('Th232') * AVOGADRO
    ...
    if lumps_FP_per_fission > 0.0:
        eta_delayed = nu_delayed / lumps_FP_per_fission
    else:
        eta_delayed = 0.0

    lambda_sd = np.log(2.0) / (5e-4)

    lambda_FP = np.log(2.0) / 1.0
    ...
    E_FISSION_PROMPT = 180.0e6 * e_charge
    E_FP_DECAY = 10.0e6 * e_charge
    E_SLOWDOWN = E_fast_eV * e_charge
    ...
    for k in range(n_steps):
        ...
        E_cum += P_total * dt
        burnup[k] = E_cum / (mTot * 1e6 * 86400.0)
